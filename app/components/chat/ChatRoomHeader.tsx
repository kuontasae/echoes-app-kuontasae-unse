...
export function ChatRoomHeader({
  ...
  const title = isCommunity
    ? community?.name || labels.communityFallback
    : isGroup
      ? group?.name || labels.groupFallback
  ...
  return (
    <div className="flex items-center p-3 bg-[#0a0a0a]/95 backdrop-blur-md sticky top-0 border-b border-zinc-900 z-10">
      <button onClick={onBack} aria-label={labels.backAria} className="p-2 -ml-1 text-white hover:opacity-80 transition-opacity"><IconChevronLeft /></button>
      <div className="flex-1 overflow-hidden px-2 flex flex-col">
        <h2 className="text-white font-bold text-[16px] truncate">{title}</h2>
        {(isCommunity || isGroup) && (
          <p className="text-[11px] font-bold text-zinc-500 truncate">{labels.membersCount.replace('{count}', String(memberCount))}</p>
        )}
      </div>
      {(isCommunity || isGroup) ? (
        <button onClick={onOpenDetails} aria-label={labels.detailsAria} className="p-2 -mr-1 text-white hover:opacity-80 transition-opacity">

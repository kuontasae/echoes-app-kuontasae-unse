    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-[900] flex items-center justify-center p-6 animate-fade-in" onClick={onClose}>
      <div className="bg-[#1c1c1e] border border-zinc-800 p-6 rounded-3xl w-full max-w-sm shadow-2xl flex flex-col max-h-[70vh]" onClick={e => e.stopPropagation()}>
        ...
        <div className="relative mb-4 shrink-0">
          ...
          <input type="text" placeholder={searchPlaceholder} value={searchQuery} onChange={(e) => onSearchChange(e.target.value)} className="w-full bg-black border border-zinc-800 rounded-lg py-2 pl-10 text-xs text-white focus:outline-none" />
        </div>
          ...
          {users.map(u => (
            <div key={u.id} className="flex items-center justify-between">
              <div className="flex items-center gap-3 cursor-pointer" onClick={() => onOpenUser(u)}>
                <img src={u.avatar} alt="" className="w-10 h-10 rounded-full object-cover border border-zinc-800" />
                <div><p className="font-bold text-xs flex items-center">{u.name}</p><p className="text-[10px] text-zinc-500">@{u.handle}</p></div>
              </div>
              <button onClick={() => onToggleFollow(u.id)} className={`px-4 py-1.5 rounded-full text-[9px] font-bold uppercase tracking-wider ${followedUsers.has(u.id) ? 'border border-zinc-700 text-zinc-400' : 'bg-white text-black'}`}>{followedUsers.has(u.id) ? followingLabel : followLabel}</button>
            </div>
          ))}
        </div>
      </div>

    ...
    <main className="min-h-screen bg-black text-white pb-24 font-sans relative selection:bg-zinc-800 overflow-x-hidden">
      ...
      {activeAlbumProfile && (
        <AlbumDetailOverlay
          ...
          canSendSong={Boolean((activeTab === 'chat' && activeChatUserId) || (activeTab === 'other_profile' && viewingUser))}
          labels={{
            tracksLoading: t('artistTracksLoading'),
          }}
          onBack={() => { setAlbumSongs([]); setPlayingSong(null); audioRef.current?.pause(); handleGoBack(); }}
          onSelectTrack={(tItem) => { if ((activeTab === 'chat' && activeChatUserId) || (activeTab === 'other_profile' && viewingUser)) { if (activeTab === 'other_profile' && viewingUser) setActiveChatUserId(viewingUser.id); setSelectedChatSong(tItem); setShowChatMusicSelector(true); } else { setDraftSong(tItem); } }}
          onSendTrack={(tItem) => {
      ...
      {selectedCalendarPopupVibe && (
        <CalendarPopupVibeOverlay
          vibe={selectedCalendarPopupVibe}
          viewArtistDetailLabel={t('viewArtistDetail')}
          onClose={() => setSelectedCalendarPopupVibe(null)}
          onOpenArtist={(e) => handleArtistClick(e, selectedCalendarPopupVibe.artistId, selectedCalendarPopupVibe.artist, selectedCalendarPopupVibe.imgUrl)}
        />
      )}
      ...
      {showAdminDashboard && (
        (() => {
          const reportedCommunities = realCommunities
            .filter(c => (c.reportedBy?.length || 0) >= 3)
            .map(c => ({
              id: c.id,
              name: c.name,
              date: c.date,
              memberCount: c.memberCount,
              reportCount: c.reportedBy?.length || 0,
            }));

          return (
            <AdminDashboard
              communities={reportedCommunities}
              labels={{
                title: t('adminDashboard'),
                descriptionLine1: t('adminDashboardDescriptionLine1'),
                descriptionLine2: t('adminDashboardDescriptionLine2'),
                reportCount: t('adminReportCount'),
                participantsCount: t('adminParticipantsCount'),
                deletePermanently: t('adminDeletePermanently'),
                restoreSafe: t('adminRestoreSafe'),
                noReportedCommunities: t('adminNoReportedCommunities'),
                peacefulState: t('adminPeacefulState'),
              }}
